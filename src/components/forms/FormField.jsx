...
import { KIND_INPUT_PROPS, sanitize, validateField } from '../../utils/validators';
...
function FieldWrapper({ id, label, required, error, hint, dark, children }) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {error && (
        <p id={`${id}-error`} className={cn('flex items-center gap-1 text-xs font-medium', dark ? 'text-[var(--color-gold)]' : 'text-[var(--color-gold)]')} role="alert">
 ...
 * `kind`: what the field may contain — 'name' (letters only), 'alnum' (letters +
 *   numbers), 'code', 'username', 'integer', 'decimal', 'digits', 'phone'
 *   (11–12 digits) or 'email'. Invalid characters are removed as the person
 *   types or pastes; see src/utils/validators.js.
 */
export const Input = forwardRef(function Input(
  { label, error, hint, required, id, icon: Icon, trailing, dark = false, className = '', kind, type, onChange, onBlur, placeholder, ...props },
  ref
) {
  const autoId = useId();
  const inputId = id || autoId;
  const { t } = useApp();
  // Message shown right after leaving a phone/email field with a wrong value.
  const [blurError, setBlurError] = useState('');

  const handleChange = (e) => {
    if (kind) {
      const cleaned = sanitize(kind, e.target.value);
  ...
  const handleBlur = (e) => {
    if (kind === 'phone' || kind === 'email') setBlurError(validateField(kind, e.target.value, t));
    onBlur?.(e);
  };
  const shownError = error || blurError;
  const kindProps = kind ? KIND_INPUT_PROPS[kind] : undefined;
  const shownPlaceholder = placeholder ?? (kind === 'phone' ? '250788123456' : undefined);

  return (
    <FieldWrapper id={inputId} label={label} required={required} error={shownError} hint={hint} dark={dark}>
      <div className="relative">
        ...
        <input
          ...
          className={cn(fieldClasses(!!shownError, dark), Icon && 'pl-10', trailing && 'pr-10', className)}
          {...kindProps}
          {...props}
