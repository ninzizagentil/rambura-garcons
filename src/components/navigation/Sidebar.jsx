...
import { ROLE_LABELS, getNavForUser } from '../../data/roles';
...
function translateLabel(label, t) {
  const labelMap = {
    ...
    'Management & Website': 'managementWebsite',
...
function NavItem({ item, collapsed }) {
  ...
  // Auto-open the group when navigation enters one of its children. Adjusted
  // during render (only on the false -> true transition) so the user can still
  // collapse it manually — see https://react.dev/learn/you-might-not-need-an-effect
...
export default function Sidebar() {
  ...
  const navItems = useMemo(() => getNavForUser(user, hasPermission), [user, hasPermission]);
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 overflow-y-auto px-2.5 py-4">
        <div className="space-y-1.5">
          {navItems.map((item) => (
            <NavItem key={`${item.to}-${item.label}`} item={item} collapsed={sidebarCollapsed} />
          ))}
        </div>
      </nav>
