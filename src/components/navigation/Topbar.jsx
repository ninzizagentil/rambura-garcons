...
import { getNavForUser } from '../../data/roles';
...
const NAV_LABEL_KEYS = {
  ...
  'Library Dashboard': 'libraryDashboard', 'Stock Dashboard': 'stockDashboard', 'Equipment Dashboard': 'equipmentDashboard', 'Management Dashboard': 'managementDashboard', 'Equipment Records': 'equipmentRecords',
  Books: 'books', 'Borrowed Books': 'borrowedBooks', 'Overdue Books': 'overdueBooks', Returns: 'returns',
  'Borrowing History': 'borrowingHistory', 'Library Reports': 'libraryReports', Applications: 'applications', 'Contact Messages': 'contactMessages', 'Developers Page': 'developersPage',
...
function useSearchableItems() {
  ...
  return useMemo(() => {
    const items = getNavForUser(user, hasPermission);
    const flat = [];
...
function GlobalSearch() {
  ...
  // Changing the query always resets the highlighted result (done in the
  // handler that changes it, not in an effect).
  const updateQuery = (value) => { setQuery(value); setActiveIndex(0); };
