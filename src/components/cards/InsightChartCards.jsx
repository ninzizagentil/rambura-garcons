...
export function InsightCard({ icon: Icon, question, answer, onClick, tone = 'default' }) {
  const toneStyles = {
    default: 'bg-[var(--color-light-green-100)] text-[var(--color-heading)]',
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full text-left bg-[var(--color-white)] rounded-[var(--radius-card)] border border-[var(--color-border-gray)] p-5 shadow-card hover:shadow-card-hover transition-shadow"
    >
...
export function ChartCard({ title, description, actions, children, className = '' }) {
  return (
    <div className={cn('bg-[var(--color-white)] rounded-[var(--radius-card)] border border-[var(--color-border-gray)] p-5 shadow-card', className)}>
...
export function FormSection({ title, description, children }) {
  return (
    <fieldset className="border-t border-[var(--color-border-gray)] pt-5 first:border-t-0 first:pt-0">
      ...
      {title && <h3 className="font-display text-sm font-semibold text-[var(--color-heading)] mb-1">{title}</h3>}
