...
const DEFAULT_CONTACT_INFO = {
  ...
  mapQuery: 'Rambura Garcons TVET School, Nyabihu, Rwanda',
};
...
export async function refreshContent() {
  ...
  loading = Promise.allSettled([
    ...
    const [hero, programs, departments, staff, news, gallery, settings] = values;
    if (hero) cache.hero = hero.data || {};
    if (programs) cache.programs = list(programs);
    if (departments) cache.departments = list(departments);
    if (staff) cache.staff = list(staff);
    if (news) cache.news = list(news);
    if (gallery) cache.gallery = list(gallery);
    if (settings) cache.settings = settings.data || {};
    contentLoaded = true;
...
export function getContactInfo() {
  return { ...DEFAULT_CONTACT_INFO, ...(cache.settings.contact || cache.settings.contactSettings || {}) };
}
export function getContactSettings() { return getContactInfo(); }
export function getSystemSettings() {
  return {
    schoolName: cache.settings.schoolName || 'Rambura Garçons TVET School',
    district: cache.settings.district || 'Nyabihu',
    contactEmail: cache.settings.email || 'info@ramburagarcons.rw',
    loanPeriodDays: String(cache.settings.loanPeriodDays || 14),
    lowStockDefault: String(cache.settings.lowStockDefault || 10),
    aboutText: cache.settings.aboutText || 'A Technical and Vocational Education and Training school in Nyabihu District, Rwanda.',
  };
}
...
export function updateSystemSettings(data) {
  return mutate('put', '/admin/settings', {
    schoolName: data.schoolName,
    district: data.district,
    email: data.contactEmail,
    loanPeriodDays: Number(data.loanPeriodDays),
    lowStockDefault: Number(data.lowStockDefault),
...
export async function updateDevelopersPage(data) {
  try {
    ...
    return mutate('put', '/admin/developers-page', { developersPage: { ...data, developers } });
  } catch (error) {
