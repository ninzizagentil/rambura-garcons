    ...
    <div>
      ...
      <div className="bg-[var(--color-white)] rounded-[var(--radius-card)] border border-[var(--color-border-gray)] divide-y divide-[var(--color-border-gray)]">
        {notifications.length === 0 ? (
          ...
          notifications.map((n) => <NotificationItem key={n.id} notification={n} onClick={handleClick} onDelete={(item) => { deleteNotification(item.id); showToast(t('notificationDeleted'), 'success'); }} />)
        )}
      </div>
    </div>
  );
