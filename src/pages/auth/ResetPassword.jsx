...
export default function ResetPassword() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!params.get('token')) { setError('This reset link is missing its security token.'); return; }
    if (password.length < 8) { setError('Password must be at least 8 characters.'); return; }
    if (password.length > 200) { setError('Password must be 200 characters or fewer.'); return; }
        ...
        {status === 'success' ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            <Input
              ...
              minLength={8}
              maxLength={200}
              error={error && !confirmation ? error : ''}
              value={password}
            ...
            <Input
              ...
              minLength={8}
              maxLength={200}
              error={error && confirmation && password !== confirmation ? error : ''}
              value={confirmation}
