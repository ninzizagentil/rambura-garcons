...
export default function Transactions() {
  ...
  const filtered = useMemo(() => {
    return transactions.filter((t) => {
      const matchesSearch = !search || t.itemName.toLowerCase().includes(search.toLowerCase()) || t.party.toLowerCase().includes(search.toLowerCase());
  ...
  const columns = [
    ...
    { key: 'quantity', header: 'Quantity', render: (t) => `${t.type === 'in' ? '+' : '−'}${t.quantity}` },
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Complete record of all Stock In and Stock Out movements."
        breadcrumb={[{ label: 'Stock', to: '/stock' }, { label: 'Transactions' }]}
      />
      <div className="flex flex-wrap gap-3 mb-4">
        <SearchBar value={search} onChange={setSearch} placeholder="Search by item or party…" className="flex-1 min-w-[220px]" />
