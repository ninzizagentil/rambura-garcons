...
export default function Suppliers() {
  ...
  const { user } = useAuth();
  const { viewOnly } = useModuleAccess(ROLES.STOCK_MANAGER, 'stock', ['stock.suppliers']);
  const { t } = useApp();
  ...
  // History rows are stored together with the supplier id they belong to, so
  // stale rows are never shown for another supplier and closing the dialog
  // needs no reset effect.
  const [historyFetch, setHistoryFetch] = useState({ id: null, rows: [] });
  const historyRows = historyTarget && historyFetch.id === historyTarget.id ? historyFetch.rows : [];
  useEffect(() => {
    if (!historyTarget) return undefined;
    let cancelled = false;
    getSupplyHistoryForSupplier(historyTarget.id).then((rows) => {
      if (!cancelled) setHistoryFetch({ id: historyTarget.id, rows });
    });
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <SearchBar value={search} onChange={handleSearchChange} placeholder={t('searchSupplierContact')} className="flex-1 min-w-[220px]" />
      </div>
