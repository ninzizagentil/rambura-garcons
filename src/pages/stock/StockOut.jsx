...
export default function StockOut() {
  ...
  const { user } = useAuth();
  const { viewOnly } = useModuleAccess(ROLES.STOCK_MANAGER, 'stock', ['stock.out']);
  const { t } = useApp();
  ...
  const validate = () => {
    ...
    if (!form.party?.trim()) next.party = t('destinationUseRequired');
  ...
  const handleConfirm = async () => {
    ...
    if (!res.success) {
      ...
      setResult(res);
      setStep('success');
      showToast(t('stockOutRecordedFor', { name: selectedItem.name }), 'success');
      ...
      logActivity({
        ...
        action: t('recordedStockOutActivity', { name: selectedItem.name, quantity: form.quantity, unit: selectedItem.unit }),
        module: 'Stock',
  ...
  if (viewOnly) {
    return (
      <div>
        <PageHeader title={t('stockOut')} description={t('issueStockDescription')} breadcrumb={[{ label: t('stockManagement'), to: '/stock' }, { label: t('stockOut') }]} />
        ...
        <div className="bg-[var(--color-white)] rounded-[var(--radius-card)] border border-[var(--color-border-gray)] p-12 flex flex-col items-center text-center">
          <p className="text-sm text-[var(--color-mid-gray)]">{t('stockOutReserved')}</p>
  ...
  if (step === 'success') {
    return (
      <div>
        <PageHeader title={t('stockOut')} breadcrumb={[{ label: t('stockManagement'), to: '/stock' }, { label: t('stockOut') }]} />
        <div className="stock-success-state">
          <CheckCircle2 className="stock-success-icon" aria-hidden="true" />
          <h3 className="stock-success-title">{t('stockOutRecorded')}</h3>
          <p className="stock-success-message">
            {t('stockOutSuccessMessage', { quantity: form.quantity, unit: selectedItem?.unit, name: selectedItem?.name })}
          </p>
          <div className="stock-success-details">
            ...
            <div className="stock-success-detail-row">
              <span className="stock-success-detail-label">{t('quantityIssued')}</span>
              <span className="stock-success-detail-value">−{form.quantity} {selectedItem?.unit}</span>
            </div>
            <div className="stock-success-detail-row">
              <span className="stock-success-detail-label">{t('remaining')}</span>
              <span className="stock-success-detail-value">{result?.newQuantity} {selectedItem?.unit}</span>
            </div>
            <div className="stock-success-detail-row">
              <span className="stock-success-detail-label">{t('destination')}</span>
      ...
      <div className="stock-form-container">
        {step === 'form' ? (
          <form onSubmit={handleContinue} noValidate className="space-y-4">
            <h3 className="stock-form-title">{t('recordStockOut')}</h3>
            ...
            <Input kind="alnum" label={t('destinationUse')} required value={form.party} onChange={update('party')} error={errors.party} placeholder={t('destinationUsePlaceholder')} />
            <Input kind="name" label={t('responsibleUser')} required value={form.responsibleUser} onChange={update('responsibleUser')} error={errors.responsibleUser} />
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Button type="submit" variant="primary" icon={PackageMinus}>{t('continue')}</Button>
            </div>
          </form>
        ) : (
          <div className="space-y-4">
            ...
            <dl className="text-sm bg-[var(--color-off-white)] rounded-[var(--radius-control)] p-4 space-y-1.5">
              ...
              <div className="flex justify-between"><dt className="text-[var(--color-mid-gray)]">{t('destinationUse')}</dt><dd className="font-medium">{form.party}</dd></div>
            ...
            <div className="flex justify-end gap-3">
              ...
              <Button variant="primary" onClick={handleConfirm} loading={saving}>{t('confirmStockOut')}</Button>
            </div>
          </div>
        )}
      </div>
