...
export default function StockAdjustment() {
  ...
  const { user } = useAuth();
  const { viewOnly } = useModuleAccess(ROLES.STOCK_MANAGER, 'stock', ['stock.adjust']);
  const { t } = useApp();
      ...
      <div className="stock-form-container">
        {step === 'form' ? (
          <form onSubmit={handleContinue} noValidate className="space-y-4">
            ...
            <Input kind="decimal" label={t('physicalQuantityCounted')} type="number" min="0" required value={form.physicalQuantity} onChange={update('physicalQuantity')} error={errors.physicalQuantity} />
