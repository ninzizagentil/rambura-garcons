...
export default function DamageDisposal() {
  ...
  const { viewOnly } = useModuleAccess(ROLES.STOCK_MANAGER, 'stock', ['stock.damage', 'stock.dispose.request', 'stock.dispose.approve']);
...
function DamagedTab({ viewOnly, t }) {
  ...
  const columns = [
    ...
    { key: 'date',        header: t('date'),        render: (d) => formatStockDate(d.date) },
    { key: 'reportedBy',  header: t('reportedBy'), render: (d) => d.reportedBy || 'Unknown user' },
...
function DisposedTab({ showToast, t, language }) {
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      // `api` refreshes an expired access token automatically (raw fetch did not).
      const json = await api.get('/stock/disposed', { page, limit, status: filter !== 'all' ? filter : undefined });
  ...
  // Synchronizes the table with the server whenever page/filter change — a real
  // external-system sync; the loading flag is set synchronously on purpose.
  ...
  const columns = [
    ...
    { key: 'requestedBy', header: t('requestedBy'), render: (r) => r.requestedBy?.fullName || t('system') },
    { key: 'approvedBy',  header: t('approvedBy'),  render: (r) => r.approvedByName || r.approvedBy?.fullName || '—' },
...
function ApprovalsTab({ viewOnly, showToast, user }) {
  ...
  const fetchPending = async () => {
    setLoading(true);
    try {
      const json = await api.get('/stock/disposal-requests/pending', { page, limit: 10 });
  ...
  // Same as above: re-fetch the pending list when the page changes.
  // oxlint-disable-next-line react/set-state-in-effect
  useEffect(() => { fetchPending(); }, [page]); // eslint-disable-line react-hooks/exhaustive-deps

  const handleAction = async () => {
    setProcessing(true);
    const endpoint = `/stock/disposal-requests/${selected._id}/${actionType}`;
  ...
  const columns = [
    ...
    { key: 'requestedBy',     header: 'Requested By', render: (r) => r.requestedBy?.fullName || 'Unknown' },
        ...
        {selected && !actionType && (
          <div className="space-y-4">
            <dl className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><dt className="text-[var(--color-mid-gray)] font-semibold">{t('requestedBy')}</dt><dd className="mt-1">{selected.requestedBy?.fullName || t('system')}</dd></div>
...
function ReconciliationTab({ viewOnly, showToast, user }) {
  ...
  const loadReconciliations = async () => {
    try {
      const json = await api.get('/stock/reconciliations');
      setReconciliations(json.data || []);
    } catch (err) { if (err.status !== 403) showToast(err.message, 'error'); }
    finally { setLoading(false); }
  };
  // Manual refresh (after create etc.) shows the spinner again first.
  const fetch_ = () => { setLoading(true); return loadReconciliations(); };

  // First load from the server (external-system sync); `loading` already starts true.
  // oxlint-disable-next-line react/set-state-in-effect
  useEffect(() => { loadReconciliations(); }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const handleCreate = async (e) => {
    e.preventDefault();
    if (!form.title || !form.location || !form.scheduledDate) { showToast(t('dateRequired'), 'error'); return; }
    const titleProblem = validateField('alnum', form.title, t);
    if (titleProblem) { showToast(titleProblem, 'error'); return; }
    setCreating(true);
    try {
      await api.post('/stock/reconciliations', { ...form, scheduledDate: new Date(form.scheduledDate) });
  ...
  const handleComplete = async () => {
    ...
    const quantityProblem = itemsData.map((i) => validateField('decimal', String(i.physicalQuantity ?? ''), t)).find(Boolean);
    if (quantityProblem) { showToast(quantityProblem, 'error'); return; }
    setCreating(true);
    try {
      await api.post(`/stock/reconciliations/${selected._id}/start`);
      await api.post(`/stock/reconciliations/${selected._id}/complete`, {
        items: itemsData,
        ...
        <div className="rounded-[var(--radius-card)] border border-[var(--color-border-gray)] bg-[var(--color-white)] p-5">
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Input kind="alnum" label={t('title')} value={form.title}
                    ...
                    {itemsData.map((item, idx) => (
                      <div key={idx} className="rounded-lg border border-[var(--color-border-gray)] p-4 space-y-3">
                        ...
                        <div className="grid grid-cols-3 gap-3">
                          <Input kind="decimal" label={t('systemQuantity')} type="number" min="0" value={item.systemQuantity}
                            onChange={(e) => handleUpdateItem(idx, 'systemQuantity', Number(e.target.value))} />
                          <Input kind="decimal" label={t('physicalQuantity')} type="number" min="0" value={item.physicalQuantity}
