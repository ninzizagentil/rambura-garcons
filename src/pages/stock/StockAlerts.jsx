...
function formatExpiryDate(value) {
  if (!value) return '—';
  const date = String(value).slice(0, 10).split('-').map(Number);
  if (date.length !== 3 || date.some(Number.isNaN)) return String(value);
  return new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    .format(new Date(Date.UTC(date[0], date[1] - 1, date[2])));
}
...
export default function StockAlerts() {
  ...
  const expiryCols = useMemo(() => [
    ...
    { key: 'expiryDate',  header: t('expiryDate'), render: (i) => formatExpiryDate(i.expiryDate) },
