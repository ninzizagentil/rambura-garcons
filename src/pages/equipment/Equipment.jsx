...
export default function Equipment() {
  ...
  const equipmentTypeOptions = [...new Set([...types, ...items.map((item) => item.type), form.type].filter(Boolean))]
    .map((type) => ({ value: type, label: type.replaceAll('_', ' ') }));
  const equipmentLocationOptions = [...new Set([...items.map((item) => item.location), form.location].filter(Boolean))]
