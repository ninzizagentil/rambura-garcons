...
import {
  ...
  UserCheck, Building2, Target, GraduationCap, Users, BookOpen, Play, Search,
} from 'lucide-react';
    ...
    <div>
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-6 flex flex-col justify-between min-h-[560px] md:min-h-[620px] pb-0">
          ...
          <div className="-mx-4 mt-12 border-y border-white/15 bg-[rgba(6,27,23,0.46)] px-4 py-6 shadow-[0_-8px_30px_rgba(0,0,0,0.12)] backdrop-blur-xl md:-mx-6 md:px-6">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
              {(homeContent.highlights || HERO_HIGHLIGHTS).map(({ title, subtitle }, index) => {
                ...
                <motion.div
                  ...
                  className="group flex min-h-[88px] items-center gap-3 rounded-[24px] border border-white/20 bg-white/[0.1] px-3.5 py-3.5 shadow-[0_12px_30px_rgba(0,0,0,0.12)] backdrop-blur-lg transition-all duration-300 hover:-translate-y-1 hover:border-[var(--color-gold)]/55 hover:bg-white/[0.16] hover:shadow-[0_18px_42px_rgba(0,0,0,0.2)] sm:px-4"
                >
                  <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-[var(--color-gold)]/25 bg-[rgba(25,135,84,0.24)] text-[var(--color-gold)] shadow-[0_8px_16px_rgba(25,135,84,0.15)] transition-transform duration-300 group-hover:scale-105">
                    <Icon className="h-5 w-5" aria-hidden="true" />
                  </span>
                  <span className="text-sm font-semibold leading-tight text-white">
                    {title}
                    <span className="mt-1 block text-xs font-normal text-white/75">{subtitle}</span>
                  </span>
                </motion.div>
                );
              })}
          ...
          <div>
            ...
            <div className="mt-7 grid gap-4">
              {testimonials.map((testimonial) => (
                <blockquote key={testimonial.name} className="rounded-[24px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[0_16px_32px_rgba(23,59,49,0.1)] transition-all duration-300 hover:-translate-y-0.5 hover:border-[var(--color-gold)]/45 hover:shadow-[0_20px_40px_rgba(23,59,49,0.16)]">
                  <Quote className="h-5 w-5 text-[var(--color-gold)]" aria-hidden="true" />
                  <p className="mt-3 text-sm leading-relaxed text-[var(--text-primary)]">{testimonial.quote}</p>
                  <footer className="mt-4 text-xs font-semibold text-[var(--text-secondary)]">{testimonial.name} · {testimonial.detail}</footer>
                </blockquote>
              ))}
            </div>
          </div>
          <div className="rounded-[28px] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[0_20px_45px_rgba(23,59,49,0.12)] md:p-8">
            <div className="flex items-center gap-3"><Trophy className="h-6 w-6 text-[var(--color-gold)]" aria-hidden="true" /><p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--text-secondary)]">{progress.eyebrow}</p></div>
            <h2 className="mt-4 font-display text-2xl font-bold text-[var(--text-primary)]">{progress.title}</h2>
            <div className="mt-7 space-y-5">
              {achievements.map((achievement) => (
                <div key={achievement.label} className="flex items-start gap-4 border-t border-[var(--border)] pt-4 first:border-t-0 first:pt-0">
                  <span className="min-w-14 font-display text-2xl font-bold text-[var(--color-gold)]">{achievement.value}</span>
                  <span><span className="block text-sm font-semibold text-[var(--text-primary)]">{achievement.label}</span><span className="mt-1 block text-xs text-[var(--text-secondary)]">{achievement.year}</span></span>
                </div>
              ))}
            </div>
          </div>
      ...
      <section className="pb-16 max-w-7xl mx-auto px-4 md:px-6">
        <div className="relative overflow-hidden rounded-[32px] border border-[var(--border)] bg-[var(--surface)] px-6 py-8 md:px-10 md:py-10 flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_18px_40px_rgba(23,59,49,0.14)]">
          ...
          <div className="relative text-center md:text-left">
            ...
            <h2 className="font-display text-2xl md:text-3xl font-bold text-[var(--text-primary)]">{homeContent.admissionsCta?.title}</h2>
            <p className="text-[var(--text-secondary)] mt-2 max-w-md">{homeContent.admissionsCta?.description}</p>
          </div>
          <div className="relative shrink-0 flex flex-col sm:flex-row items-center gap-3">
            ...
            <Link
              to="/admissions#track-application"
              className="inline-flex items-center gap-2 rounded-full border border-[var(--color-border-gray)] bg-white/70 px-5 py-2.5 text-sm font-semibold text-[var(--color-dark-gray)] shadow-sm transition hover:border-[var(--color-gold)] hover:text-[var(--color-deep-green)] hover:shadow-md"
            >
