...
export default function Contact() {
  ...
  const validate = () => {
    ...
    if (form.name.trim().length > 120) next.name = 'Name must be 120 characters or fewer.';
    if (form.email && !/^\S+@\S+\.\S+$/.test(form.email)) next.email = 'Enter a valid email address.';
    if (form.email.trim().length > 160) next.email = 'Email must be 160 characters or fewer.';
    if (form.subject.trim().length > 160) next.subject = 'Subject must be 160 characters or fewer.';
    if (form.message.trim().length > 5000) next.message = 'Message must be 5,000 characters or fewer.';
    applyKindErrors(next, form, FIELD_KINDS, undefined);
    setErrors(next);
