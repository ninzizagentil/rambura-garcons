...
import { UserPlus, Eye, Pencil, UserX, UserCheck, Trash2 } from 'lucide-react';
...
export default function Users() {
  ...
  useEffect(() => {
    let cancelled = false;
    getUsers({ limit: 200 }).then((rows) => { if (!cancelled) setUsers(rows); });
  ...
  const handleDeleteUser = async () => {
    ...
    showToast(t('userDeleted', { name: target.fullName }), 'success');
    refresh();
  };
    ...
    { key: 'lastActivity', header: t('lastActivity'), render: (u) => new Date(u.lastActivity).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }) },
    {
      ...
      render: (u) => (
        <div className="flex items-center gap-1">
          ...
          {hasPermission('users.update') && <IconButton
            ...
            />}
          {hasPermission('users.delete') && <IconButton
              icon={Trash2}
              label={`${t('delete')} ${u.fullName}`}
              variant="danger"
  ...
  return (
    <div>
      ...
      <ConfirmModal
        ...
        title={t('deleteUser')}
        message={t('confirmDeleteUser', { name: deleteTarget?.fullName || '' })}
