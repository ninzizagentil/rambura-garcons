...
export function validateBody(rules) {
  return (req, res, next) => {
    ...
    for (const [field, rule] of Object.entries(rules)) {
      ...
      // requiredWhen(body) makes a field mandatory only in some cases (e.g. batch number for Foods).
      if ((rule.required || rule.requiredWhen?.(req.body || {})) && isEmpty) errors.push({ field, message: rule.message || `${field} is required` });
      if (value !== undefined && value !== null && value !== '') {
        ...
        if (rule.enum && !rule.enum.includes(value)) errors.push({ field, message: `${field} has an invalid value` });
        if (rule.validate && !rule.validate(value, req.body)) errors.push({ field, message: rule.message || `${field} has an invalid value` });
      }
    }
