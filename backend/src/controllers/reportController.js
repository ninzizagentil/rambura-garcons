...
export async function librarySummary(req, res) {
  ...
  const monthlyLoans = await Loan.aggregate([
    ...
    { $group: { _id: { $dateToString: { format: '%Y-%m', date: '$borrowDate' } }, loans: { $sum: 1 } } },
    { $sort: { _id: 1 } },
  ]);
  const summary = books[0] || { totalBooks: 0, totalCopies: 0, borrowedCopies: 0 };
  return sendReport(req, res, { ...summary, availableCopies: summary.totalCopies - summary.borrowedCopies, overdueLoans, returnedLoans, mostBorrowedBooks: mostBorrowed, monthlyLoans }, 'library-summary');
}
...
export async function stockAnalytics(req, res) {
  ...
  const [usage, category, movement] = await Promise.all([
    ...
    StockTransaction.aggregate([{ $match: { date: { $gte: since } } }, { $group: { _id: { $dateToString: { format: '%Y-%m', date: '$date' } }, received: { $sum: { $cond: [{ $eq: ['$type', 'in'] }, '$quantity', 0] } }, issued: { $sum: { $cond: [{ $eq: ['$type', 'out'] }, '$quantity', 0] } } } }, { $sort: { _id: 1 } }]),
  ]);
...
export async function equipmentSummary(req, res) {
  const start = new Date();
  start.setMonth(start.getMonth() - 4, 1);
  start.setHours(0, 0, 0, 0);
  const [totals, byStatus, byCondition, monthlyRegistrations] = await Promise.all([
    Equipment.countDocuments({ active: true }),
    Equipment.aggregate([{ $match: { active: true } }, { $group: { _id: '$status', count: { $sum: 1 } } }]),
    Equipment.aggregate([{ $match: { active: true } }, { $group: { _id: '$condition', count: { $sum: 1 } } }]),
    Equipment.aggregate([
      { $match: { active: true, purchaseDate: { $gte: start } } },
      { $group: { _id: { $dateToString: { format: '%Y-%m', date: '$purchaseDate' } }, registered: { $sum: 1 } } },
      { $sort: { _id: 1 } },
    ]),
  ]);
  const statusCounts = Object.fromEntries(byStatus.map((row) => [row._id, row.count]));
  const conditionCounts = Object.fromEntries(byCondition.map((row) => [row._id, row.count]));
  return sendReport(req, res, {
    totalEquipment: totals,
