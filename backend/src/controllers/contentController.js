...
import { invalidateMailConfig } from '../services/emailService.js';
...
export async function adminCreate(req, res) { const Model = resources[req.params.resource]; if (!Model) return fail(res, 'Unknown content resource', 404); const payload = normalizePicturePayload({ ...req.body }); if (req.params.resource === 'gallery' && !String(payload.caption || payload.title || '').trim()) return fail(res, 'A caption or title is required', 422, [{ field: 'caption', message: 'caption is required' }]); if (['programs', 'departments', 'news'].includes(req.params.resource) && !payload.slug) payload.slug = slugify(payload.title || payload.name); if (req.params.resource === 'news') { payload.author = req.user._id; payload.published = true; payload.publishedAt = payload.date || new Date(); } const item = await Model.create(payload); await recordAudit(req, { action: `${req.params.resource} created`, module: 'Website', resourceId: item._id, description: `Created ${payload.title || payload.name || 'gallery item'}` }); return ok(res, item, 'Content created', 201); }
...
export async function publishNews(req, res) { const item = await News.findByIdAndUpdate(req.params.id, { published: req.body.published !== false, publishedAt: new Date() }, { new: true }); if (!item) return fail(res, 'News not found', 404); await recordAudit(req, { action: item.published ? 'News published' : 'News unpublished', module: 'Website', resourceId: item._id, description: `${item.published ? 'Published' : 'Unpublished'} "${item.title}"` }); return ok(res, item, item.published ? 'News published' : 'News unpublished'); }
...
export async function getEmailSettings(_req, res) { const setting = await SystemSetting.findOne({ key: 'email' }); const value = setting?.value || {}; return ok(res, { host: value.host || '', port: value.port || 587, secure: Boolean(value.secure), user: value.user || '', from: value.from || '', configured: Boolean(value.password) }); }
export async function updateEmailSettings(req, res) { const current = await SystemSetting.findOne({ key: 'email' }); const value = { host: req.body.host || '', port: Number(req.body.port || 587), secure: Boolean(req.body.secure), user: req.body.user || '', from: req.body.from || '', password: req.body.password ? encryptSecret(req.body.password) : (current?.value?.password || '') }; const setting = await SystemSetting.findOneAndUpdate({ key: 'email' }, { key: 'email', value }, { new: true, upsert: true }); invalidateMailConfig(); await recordAudit(req, { action: 'Email settings changed', module: 'Settings', resourceId: setting._id, description: 'Updated SMTP delivery settings' }); return ok(res, { host: value.host, port: value.port, secure: value.secure, user: value.user, from: value.from, configured: Boolean(value.password) }, 'Email settings updated'); }
export async function updateDevelopersPageSettings(req, res) { const settings = await WebsiteSetting.findOneAndUpdate({ key: 'default' }, { $set: { developersPage: req.body.developersPage || {} } }, { new: true, upsert: true, runValidators: true }); await recordAudit(req, { action: 'Developers page changed', module: 'Website', resourceId: settings._id, description: 'Updated developers page content' }); return ok(res, settings, 'Developers page updated'); }
