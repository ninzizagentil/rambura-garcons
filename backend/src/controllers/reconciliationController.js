...
export async function approveReconciliation(req, res) {
  ...
  // Separation of duties: the person who created the count cannot approve it (administrators excepted).
  if (req.user.role !== 'admin' && String(reconciliation.createdBy) === String(req.user._id)) return fail(res, 'You cannot approve a reconciliation that you created', 403);

  const seenItems = new Set();
  const validatedItems = [];
  for (const entry of reconciliation.items) {
    const itemId = String(entry.itemId || '');
    if (!itemId || seenItems.has(itemId)) return fail(res, 'Reconciliation contains a duplicate or invalid item', 422);
    seenItems.add(itemId);
    const systemQuantity = Number(entry.systemQuantity);
    const physicalQuantity = Number(entry.physicalQuantity);
    const variance = Number(entry.variance);
    if (!Number.isFinite(systemQuantity) || !Number.isFinite(physicalQuantity) || physicalQuantity < 0 || variance !== physicalQuantity - systemQuantity || Boolean(entry.adjustmentNeeded) !== (variance !== 0)) return fail(res, 'Reconciliation quantities or variance are invalid', 422);
    const item = await StockItem.findOne({ _id: entry.itemId, active: true, location: reconciliation.location });
    if (!item || item.quantity !== systemQuantity || item.quantity + variance < 0) return fail(res, 'Stock changed or item is outside this reconciliation location', 409);
    validatedItems.push({ entry, item });
  }

  const applied = [];
  try {
    // Apply adjustments to inventory
    for (const { entry, item } of validatedItems) {
      if (entry.adjustmentNeeded && entry.variance !== 0) {
        const updated = await StockItem.findOneAndUpdate(
          { _id: item._id, active: true, quantity: item.quantity },
          { $inc: { quantity: entry.variance } },
          { new: true }
        );
        if (!updated) throw Object.assign(new Error('Stock changed while applying reconciliation'), { statusCode: 409 });
        applied.push({ id: item._id, variance: entry.variance });
      }
    }
    ...
    for (const change of applied) await StockItem.findByIdAndUpdate(change.id, { $inc: { quantity: -change.variance } }).catch(() => {});
