...
export async function updateItem(req, res) { const allowed = ['code', 'name', 'category', 'unit', 'minLevel', 'unitPrice', 'description', 'location', 'supplierId', 'batchNumber', 'expiryDate', 'active']; const updates = Object.fromEntries(Object.entries(req.body).filter(([key]) => allowed.includes(key))); const item = await StockItem.findByIdAndUpdate(req.params.id, updates, { new: true, runValidators: true }); return item ? ok(res, item, 'Stock item updated') : fail(res, 'Stock item not found', 404); }
export async function deleteItem(req, res) { const item = await StockItem.findByIdAndUpdate(req.params.id, { active: false }, { new: true }); return item ? ok(res, item, 'Stock item archived') : fail(res, 'Stock item not found', 404); }
...
export async function dispose(req, res) { const quantity = Number(req.body.quantity); const session = await mongoose.startSession(); let result; try { await session.withTransaction(async () => { const item = await StockItem.findOne({ _id: req.body.itemId, quantity: { $gte: quantity }, active: true }).session(session); if (!item || quantity <= 0) { const e = new Error('Insufficient stock or invalid quantity'); e.statusCode = 409; throw e; } const previousQuantity = item.quantity; item.quantity -= quantity; result = await DisposedStock.create([{ ...req.body, quantityRemoved: quantity, remainingQuantity: item.quantity, itemName: item.name, responsibleUser: req.user._id }], { session }); await transaction(req, item, { type: 'removed', quantity, date: req.body.date || new Date(), notes: req.body.notes }, session); result = result[0]; result.previousQuantity = previousQuantity; }); return ok(res, result, 'Stock disposed', 201); } finally { await session.endSession(); } }
